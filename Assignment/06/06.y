...
%token IDENTIFIER TYPE_IDENTIFIER AUTO_SYM BREAK_SYM CASE_SYM CONTINUE_SYM DEFAULT_SYM DO_SYM ELSE_SYM ENUM_SYM FOR_SYM IF_SYM RETURN_SYM SIZEOF_SYM STATIC_SYM STRUCT_SYM SWITCH_SYM TYPEDEF_SYM UNION_SYM WHILE_SYM GOTO_SYM
PLUSPLUS MINUSMINUS ARROW LSS GTR LEQ GEQ EQL NEQ AMPAMP BARBAR LSH RSH DOTDOTDOT LP RP LB RB LR RR COLON PERIOD COMMA EXCL STAR SLASH PERCENT AMP SEMICOLON PLUS MINUS ASSIGN NOT XOR BAR QUESTION INTEGER_CONSTANT FLOAT_CONSTANT STRING_LITERAL CHARACTER_CONSTANT CONST_SYM
%start program
...
function_definition : declaration_specifiers declarator { $$=setFunctionDeclaratorSpecifier($2, $1); } compound_statement { $$=setFunctionDeclaratorBody($3, $4); current_id=$2; }
...
;
storage_class_specifier : AUTO_SYM { $$=S_AUTO; } | STATIC_SYM { $$=S_STATIC; } | TYPEDEF_SYM {$$=S_TYPEDEF; }
			;
...
;
struct_declaration : type_specifier struct_declarator_list SEMICOLON {$$=setStructDeclaratorListSpecifier($2, $1); }
		   ;
...
| jump_statement {$$=$1;}
;
labeled_statement : CASE_SYM constant_expression COLON statement {$$=makeNode(N_STMT_LABEL_CASE, $2, NIL, $4);}
		  | DEFAULT_SYM COLON statement {$$=makeNode(N_STMT_LABEL_DEFAULT, NIL, $3, NIL);}
;

compound_statement : LR {$$=current_id; current_level++; } declaration_list statement_list RR {checkForwardReference(); $$=makeNode(N_STMT_COMPOUND, $3, NIL, $4); current_id=$2; current_level--;}
		   ;
...
;
arg_expression_list : assignment_expression {$$=makeNode(N_ARG_LIST, $1, NIL, makeNode(N_ARG_LIST_NIL, NIL, NIL, NIL));}
...
inclusive_OR_expression : exclusive_OR_expression {$$=$1;}
			| inclusive_OR_expression BAR exclusive_OR_expression {$$=makeNode(N_EXP_OR, $1, NIL, $3);}
;
